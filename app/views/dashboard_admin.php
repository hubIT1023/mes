...
try {
    ...
    ");
    $stmt->execute([$tenant_id]);
    $allPages = $stmt->fetchAll(PDO::FETCH_KEY_PAIR);
} catch (PDOException $e) {
...
try {
    ...
    $stmt->execute([$tenant_id]);
    $tenantAssets = $stmt->fetchAll(PDO::FETCH_ASSOC);
} catch (PDOException $e) {
    die("Error fetching tenant assets: " . $e->getMessage());
}
...
// --- Filter Groups & Determine UI State ---
...
<header class="sticky top-[3px] z-12 bg-white border-b border-slate-200 shadow-sm">
    <nav class="max-w-7xl mx-auto px-4 flex items-center justify-between h-10">
        ...
        <div class="flex items-center gap-3">
            <span class="text-sm text-slate-500">Tenant: <?= htmlspecialchars($tenant_id ?? 'n/a') ?></span>
            <a href="/mes/logout" class="text-sm text-slate-500 hover:text-slate-700">Log out</a>
        </div>
    </nav>
</header>
...
<div class="top-product-bar">
    <div class="container-fluid">
        <div class="product-list">
            <a href="/mes/mode-color" class="product-item <?= is_active('/mes/mode-color', $current_page ?? '') ? 'text-blue-600' : '' ?>">
...
<div class="container-fluid py-4">
    <div class="alert alert-info">
        This is a test. If you see this, PHP is working.
    </div>

    <!-- Uncomment this to see what data you have -->
    <!--
    <pre>
    <?php
    echo "Pages:\n";
