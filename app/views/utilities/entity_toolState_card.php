...
/**
 * Renders the standard shared header block used in all entity-related modals.
 *
 * @param string $prefix       Short prefix for ID generation (e.g., 'ap', 'ts', 'si')
 * @param string $csrfToken    CSRF token
 * @param string $org_id       Organization ID
 * @param bool   $showGroup    Whether to show "Group" field (default: true)
 * @param bool   $showDateTime Whether to auto-fill date/time (default: true)
 */
function renderModalContextBlock(
    ...
    <?php if ($showDateTime): ?>
        <input type="hidden" name="col_6" id="<?= $prefix ?>_modal_date_time" value="<?= htmlspecialchars($currentDateTime) ?>">
    <?php endif; ?>
...
<form>
<div class="modal-body">
    <?php renderModalContextBlock('lw', $csrfToken, $org_id, showGroup: true, showDateTime: false); ?>
...
<div class="modal-body">
    <?php renderModalContextBlock('si', $csrfToken, $org_id, showGroup: true, showDateTime: true); ?>
    <div class="mb-3">
...
<div class="modal-body">
    <?php renderModalContextBlock('ap', $csrfToken, $org_id, showGroup: false, showDateTime: true); ?>
...
<div class="modal-body">
    <?php renderModalContextBlock('ts', $csrfToken, $org_id, showGroup: true, showDateTime: true); ?>

    <div class="row mb-3">
        <div class="col">
            <select id="ts_modal_stopcause" name="col_3" class="form-control" required>
...
const actionModals = ['changeStateModal', 'standingIssueModal', 'associateAccessoriesModal', 'associatePartsModal', 'LoadWorkModal'];
    ...
    modal.addEventListener('show.bs.modal', e => {
        ...
        if (btn?.hasAttribute('data-use-stored-context') && currentEntityContext) {
            ...
            if (modalId === 'LoadWorkModal') {
                ['asset_id', 'entity', 'group_code', 'location_code'].forEach(f => {
                    const el = document.getElementById(`lw_${f}`);
                    if (el) el.value = ctx[f.replace('_', '')] ?? ctx[f];
                });
            } else if (modalId === 'standingIssueModal') {
                document.getElementById('si_modal_asset_id').value = ctx.assetId;
                document.getElementById('si_ipt_entity').value = ctx.entity;
                document.getElementById('si_modal_location').value = ctx.locationName;
                document.getElementById('si_modal_group').value = ctx.groupCode;
                ...
                // Group not shown in parts modal
