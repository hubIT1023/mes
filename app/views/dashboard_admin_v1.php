...
// ✅ Fetch ONLY real groups (exclude placeholders)
$groups = [];
try {
    $stmt = $conn->prepare("
        SELECT 
            id, group_code, location_code, group_name, location_name,
            org_id, created_at, page_id, page_name
        FROM group_location_map 
        WHERE org_id = ? 
        AND group_name != '---'  -- ✅ Exclude placeholders
...
// ✅ Fetch ALL pages (including those with only placeholders)
$allPages = [];
try {
    $pageStmt = $conn->prepare("
        ...
        ORDER BY page_id
    ");
    $pageStmt->execute([$tenant_id]);
    $allPages = $pageStmt->fetchAll(PDO::FETCH_KEY_PAIR);
} catch (PDOException $e) {
    error_log("DB error fetching pages: " . $e->getMessage());
}

// Organize pages metadata
$pages = [];
...
// ✅ SINGLE PAGE SELECTION LOGIC (URL > Session > First Page)
$selectedPageId = null;
...
elseif (isset($_SESSION['last_page_id'])) {
    $selectedPageId = (int)$_SESSION['last_page_id'];
}
// Priority 3: First available page
elseif (!empty($pages)) {
    $selectedPageId = (int)array_key_first($pages);
}
...
// Filter groups for selected page
$selectedPageGroups = [];
$currentPageHasRealGroups = false;

if ($selectedPageId !== null) {
    foreach ($groups as $g) {
        if ((int)$g['page_id'] === $selectedPageId) {
            $currentPageHasRealGroups = true;
            $selectedPageGroups[] = $g;
        }
    }
}

$showBlankCanvas = empty($pages) || !$currentPageHasRealGroups;
$selectedPageName = $pages[$selectedPageId]['page_name'] ?? 'Dashboard';

// Fetch assets for modals
$tenantAssets = [];
try {
...
<body class="bg-white text-slate-900">
    ...
    <!-- ✅ Entity Modals: Always create for ALL groups (not just when !$showBlankCanvas) -->
...
<div class="modal fade" id="deleteGroupModal" tabindex="-1">
    ...
    <script>
		...
		function openUpdateGroupModal(groupId, pageId, groupName, locationName) {
