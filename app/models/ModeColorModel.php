...
class ModeColorModel {
    ...
    public function getAll(string $orgId): array {
        $stmt = $this->conn->prepare("SELECT * FROM mode_color WHERE org_id = ? ORDER BY mode_key");
        $stmt->execute([$orgId]);
        return $stmt->fetchAll(PDO::FETCH_ASSOC);
    }

    public function find(string $orgId, int $id): ?array {
        $stmt = $this->conn->prepare("SELECT * FROM mode_color WHERE id = ? AND org_id = ?");
    ...
    public function create(string $orgId, string $mode_key, string $label, string $tailwind_class): bool {
        $stmt = $this->conn->prepare("
            ...
            VALUES (?, ?, ?, ?)
        ");
        return $stmt->execute([$orgId, $mode_key, $label, $tailwind_class]);
    }
    ...
    public function update(int $id, string $orgId, string $mode_key, string $label, string $tailwind_class): bool {
        $stmt = $this->conn->prepare("
            ...
            WHERE id = ? AND org_id = ?
        ");
        return $stmt->execute([$mode_key, $label, $tailwind_class, $id, $orgId]);
    }

    public function delete(int $id, string $orgId): bool {
        $stmt = $this->conn->prepare("DELETE FROM mode_color WHERE id = ? AND org_id = ?");
